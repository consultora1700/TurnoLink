...
import type { TopOrderCustomer } from '@/lib/api';
...
export function TopOrderCustomersList({ data }: { data: TopOrderCustomer[] }) {
  return (
    <Card className="border shadow-sm">
      <CardHeader className="pb-2">
        <CardTitle className="text-base">Top Compradores</CardTitle>
        <CardDescription className="text-xs">Los que m&aacute;s compraron este mes</CardDescription>
      </CardHeader>
            ...
            {data.map((customer, i) => (
              <div key={customer.customerId} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/30 transition-colors -mx-2">
                ...
                <div className="text-right shrink-0">
                  <p className="text-sm font-semibold tabular-nums">{customer.orders} pedidos</p>
