...
import { Plus, Trash2, ChevronDown, ChevronUp, Lightbulb, X } from 'lucide-react';
...
const MODE_GROUP_ID = '__mode__';
const PRESENCIAL_OPTION_ID = '__presencial__';
const ONLINE_OPTION_ID = '__online__';

function createModeGroup(): VariationGroup {
  return {
    id: MODE_GROUP_ID,
    label: 'Modalidad',
    type: 'single',
    required: true,
    options: [
      { id: PRESENCIAL_OPTION_ID, name: 'Presencial', priceModifier: 0, pricingType: 'absolute', durationModifier: 0 },
      { id: ONLINE_OPTION_ID, name: 'Online', priceModifier: 0, pricingType: 'absolute', durationModifier: 0 },
    ],
  };
}

export function VariationsEditor({ value, onChange, serviceMode }: VariationsEditorProps) {
  const [expanded, setExpanded] = useState(value.length > 0);

  const hasModeGroup = value.some(g => g.id === MODE_GROUP_ID);

  // Auto-remove __mode__ group when serviceMode is no longer 'ambos'
  useEffect(() => {
    if (serviceMode !== 'ambos' && hasModeGroup) {
      onChange(value.filter(g => g.id !== MODE_GROUP_ID));
    }
  }, [serviceMode]);
  ...
  const addModeGroup = () => {
    onChange([createModeGroup(), ...value]);
  ...
  const showSuggestion = serviceMode === 'ambos' && !hasModeGroup;

  return (
    <div className="space-y-2">
      {/* Suggestion banner for mode variation */}
      {showSuggestion && (
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-3 p-3 rounded-lg border border-indigo-200 dark:border-indigo-800/50 bg-indigo-50/50 dark:bg-indigo-900/20">
          <div className="flex items-start gap-2 flex-1 min-w-0">
            <Lightbulb className="h-4 w-4 text-indigo-500 mt-0.5 flex-shrink-0" />
            <p className="text-xs text-indigo-700 dark:text-indigo-300 leading-relaxed">
              Tu servicio es presencial y online. ¿Querés agregar una variación de modalidad con precios diferentes?
            </p>
          </div>
          <Button
            type="button"
            variant="outline"
            size="sm"
            onClick={addModeGroup}
            className="text-xs h-7 border-indigo-300 dark:border-indigo-700 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 flex-shrink-0"
          >
            Agregar variación
          </Button>
        </div>
      ...
      {expanded && (
        <div className="space-y-3 pl-1">
          {value.map((group) => {
            const isModeGroup = group.id === MODE_GROUP_ID;

            if (isModeGroup) {
              return (
                <div key={group.id} className="border border-indigo-200 dark:border-indigo-800/50 rounded-lg p-3 space-y-3 bg-indigo-50/30 dark:bg-indigo-900/10">
                  {/* Mode group header */}
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Badge className="bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300 border-0 text-xs">
                        Modalidad
                      </Badge>
                      <span className="text-xs text-muted-foreground">Presencial / Online</span>
                    </div>
                    <Button
                      type="button"
                      variant="ghost"
                      ...
                      className="h-7 w-7 text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 flex-shrink-0"
                      title="Eliminar variación de modalidad"
                    >
                      <X className="h-3.5 w-3.5" />
                    </Button>
                  </div>

                  {/* Mode options - fixed names, editable prices */}
                  <div className="space-y-2">
                    <Label className="text-xs text-muted-foreground">Precios por modalidad</Label>
            ...
            // Regular variation group (existing behavior)
            return (
