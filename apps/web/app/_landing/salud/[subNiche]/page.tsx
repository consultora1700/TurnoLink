...
const NICHE = 'salud';
const NICHE_LABEL = 'Salud';
const SITE_URL = 'https://turnolink.com.ar';

/** Sub-niches that have their own dedicated /salud/<slug>/page.tsx */
const CUSTOM_ROUTES = new Set(['psicologos', 'nutricionistas', 'odontologos']);

export async function generateStaticParams() {
  const slugs = await getSubNicheSlugs(NICHE);
  return slugs
    .filter((s) => !CUSTOM_ROUTES.has(s))
    .map((subNiche) => ({ subNiche }));
}
